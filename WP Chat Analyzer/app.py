...
import preprocessor , helper , show_inv_data
...
if uploaded_file is not None:
    ...
    user_df = show_inv_data.show_individual_data(selected_user , df)
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        fig , ax = plt.subplots()
        plt.plot(timeline['time'] , timeline['messages'] , color = 'red')
        ...
        #Find out the busiest user in the group(Group Level)
        if selected_user == 'overall':
            ...
            with col1:
                ...
                st.pyplot(fig)
            with col2:
                st.header('Percentage Of Most Active user in grp')
                st.dataframe(new_df)
